        ...
        return arguments.length < 8 && (h = 0), u.invert = function(u, s) {
            ...
            do {
                var d = l * l,
                    ...
                    S = Math.asin(b / m) / ba,
                    Q = v * S;
                if (xB2 = b * b, dxBdf = (2 * n + d * (4 * a + 6 * d * r)) * l, dyBdf = e + d * (3 * o + 5 * d * i), dpdf = 2 * (b * dxBdf + p * (dyBdf - 1)), dqdf = 2 * (dyBdf - 1), dmdf = (dpdf * q - w * dqdf) / (q * q), cosa = Math.cos(Q), sina = Math.sin(Q), mcosa = m * cosa, msina = m * sina, da df = v / ba * (1 / M(1 - xB2 / y)) * (dxBdf * m - b * dmdf) / y, fx = msina - u, fy = l * (1 + d * h) + m - mcosa - s, dxdf = dmdf * sina + mcosa * da df, dxd? = mcosa * S, dydf = 1 + dmdf - (dmdf * cosa - msina * da df), dyd? = msina * S, denominator = dxdf * dyd? - dydf * dxd?, !denominator) break;
                v -= c = (fy * dxdf - fx * dydf) / denominator, l -= f = (fx * dyd? - fy * dxd?) / denominator
